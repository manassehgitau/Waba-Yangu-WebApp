...
def customerdashboard(request, customer_id):
    try:
        ...
        #
        # # Fetch water usage for the last 7 days (for the graph)
        # water_usage_week =WaterUsage.objects.filter(customer=customer, date=today).first()
        # water_usage_week = water_usage_today.usage if water_usage_today else 0
        #
        # # Fetch balance trend for the last 7 days
        # balance_trend_week = PrepaidBalance.objects.filter(customer=customer, date=today).first()
        # balance_trend_week = prepaid_balance.balance if prepaid_balance else 0
